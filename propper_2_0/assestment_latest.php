	...
	<div id="page_container"><!--page container-->
		...
		<div id="page_content_assess"><!--content-->
			...
			<div id="left_column"><!--left column-->
				...
				$iid = $_SESSION["uSeRiD"];
		$qunit = "select unit_type from csdm2.t_user where employee_code = $iid limit 0,1
union all
select unit_type from csdm2.t_appprofile where employee_code = $iid limit 0,1";	
		$qqu = $dbGate->ExecuteQuery($qunit,"pnp");
		$rrs = mysql_fetch_array($qqu);
		//echo $qunit;
		$unitType = $rrs[0];
		if($unitType == 'gerai' || $unitType == 'grapari' || $unitType == 'plasa' )
		{
			$unitType = 'GRAPARI';
		}
		...
		#echo "set_id:$setid|level:$segment|regional:".$_SESSION['ReGioNiD']."</br>";
            ...
            while($row = mysql_fetch_row($nbsoalres)){
				...
				$qcb = "select jenis_soal,bobot_nilai from dev_job_value where unit_type = '$unitType' and group_job_function = '$gjfr'";
			...
			#echo "assess_id:".$_SESSION["aSSeSSTYPe"]."|set_id:$setid|level:$segment|cat_group:".$row[0]."|assess_id:".$_SESSION["aSSeSSTYPe"]."|regional:".$_SESSION['ReGioNiD']."|limit:".$row[1];
				...
				$quest  =   "INSERT INTO dev_quiz 
								...
								FROM
									dev_category ca,
									dev_category_distribution cd,
									dev_question_active qa,
									option_sequence_index osi
									WHERE cd.cat_group = ca.cat_group
									AND cd.`cat_group` = qa.`cat_id`
									AND qa.set_id = cd.set_id
									AND osi.nb_option = qa.nb_option
									AND qa.set_id = $setid
									AND qa.level = $segment
									AND cd.cat_group = '".$row[0]."'
			...
			echo "ini loh $lims, $lprod, $lpros, $lpril";
			print_r ($quest);
            $ipaddress = "";
            ...
            if (mysql_num_rows($quizres) <= 0){
                ...
                $sqlscore = "INSERT INTO dev_score ".
                ...
                $sqldetail ="INSERT INTO dev_score_detail ".
            ...
            if ($arrOfRow[1] == 0){
                ...
                if (mysql_num_rows($quizres) <= 0){
                    ...
                    echo "<div id='questionerror'>Questions is not enough! Contact Administrator (Willy Badawi)</div>";
