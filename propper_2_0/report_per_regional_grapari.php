	...
	$gatesms = new DBGateway($conf->Host192,$conf->User192, $conf->Pass192);
		...
		if ($nbcsr > 0){
			...
			$nbcsrsmsna++;
				...
				/*if($nbcsr-$nbnotassesscsr > 0){
					$avgcsr = round($total/($nbcsr-$nbnotassesscsr),2);
				}
				else{
					if ($nbcsr > 0){
						$avgcsr = "0";
		...
		foreach($arrOfBO as $boid => $boname)
			...
			$totalsmsbo2 += $smsscore2;
				...
				$nbnotassessboall+=$nbnotassessbo;
				$nbnotassessboallsms+=$nbnotassessbosms;
				$nbboall+=$nbbo;
				$totalboall+=$total;
		...
		<div id="page_content_assess"><!--content-->
			<div id="left_column"><!--left column-->
 			...
 			if (($regional != "0") && ($regional != "")){
    			...
    			foreach ($arrOfShopKPI as $unitid => $kpiinfo){
    				list($gname, $nbcsr, $kpicsrsms1, $kpicsrsms2, $kpicsr, $nbbo, $kpibosms1, $kpibosms2, $kpibo, $kpi) = split("#", $kpiinfo);
    				echo "<tr>
    							<th>$gname</th>
    							<td>$nbbo</td>
    							<td>$nbcsr</td>
	            ...
	            $logger->setDBConnection($gatepnp);
	     		...
	     		$logger->Logging(HistoryType::HISTORYTYPE_VIEW_REPORT_GRAPARI_REGIONAL,$userid,"View Report per Regional GraPARI $regional Periode $selmonth - $selyear",$ipaddress);
					}
					?>   	
			</div><!--left column-->
			...
			<div id="right_column"><!--right column-->
  				<div class="section"><!--latest news-->
  					...
  					<form action="report_per_regional_grapari.php" method="post">
  					<?php
				try{
  					$channel = "SELECT 
					  				reg_code, 
