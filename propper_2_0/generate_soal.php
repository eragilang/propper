...
<?php
	...
	if ($_POST["unittype"]=="GRAPARI"){
		$where = "WHERE unit_type in ('grapari','gerai','plasa')";
	}else{
		$where = "WHERE unit_type = '".$_POST["unittype"]."'";
	}
	$upd_ass_period = "UPDATE assess_period SET start_assess = ".$start_assess.", end_assess = ".$end_assess." ".$where."";
	$res_upd_ass_period = mysql_query($upd_ass_period);
	
	$cekJobLevel = "SELECT LEVEL, level_type, TYPE, description FROM `job_level` GROUP BY 1,2,3 ORDER BY 1";
	$resCekJobLevel = mysql_query($cekJobLevel);
	
	#DELETE QUESTION ACTIVE
	if ($_POST["unittype"]=="GRAPARI"){
		$setid = 1;
	}else if($_POST["unittype"]=="WEBSERVICE") {
		$setid = 2;
	}else{
		$setid = 3;
	}
	$delSoalActive = "DELETE FROM `question_active` WHERE set_id='".$setid."'";
	...
	$jml_row = 0;
		while($rowCekJobLevel = mysql_fetch_array($resCekJobLevel)){
			...
			$desc = $rowCekJobLevel[3];
			
			$chk_min_soal = "SELECT DISTINCT max_question FROM assess_param WHERE level = '".$level."' AND set_id = '".$set_id."';";
			...
			a.* FROM question_temp a WHERE a.LEVEL = '$level_type'";
			//echo $chk_jml_soal;
			...
			if($jml_soal > $min_soal){
				...
				if ($_POST["unittype"]=="CALLCENTER"){
					...
					$cekCatSoal = "SELECT cat_id FROM `dev_category_ryan` WHERE description = '$level_type' AND group_description = '$jenis'";
					//echo $cekCatSoal;
			...
			//print_r ($cekCatSoal);
					...
					if($jumlahCatSoal > 0){
						#echo "ADA \n";
						$insertSoalActive = "INSERT INTO question_active VALUES(
						'".$rowInsertQuest[0]."',
						'".$setid."',
						'".$rowInsertQuest[1]."',
						'".$rowInsertQuest[2]."',
						...
						$insertSoalHistory = "INSERT INTO question_history VALUES(
						'".$rowInsertQuest[0]."',
						'".$setid."',
						'".$rowInsertQuest[1]."',
						'".$rowInsertQuest[2]."',
						...
						if($resInsertSoalActive){
							...
							//truncate table question temp
							$trunc = "TRUNCATE TABLE QUESTION_TEMP";
							mysql_query($trunc);
						}else{
	...
	$upd_qs_active = "UPDATE question_active SET 
	...
	$upd_qs_active_qst = "UPDATE question_active 
						SET 
	...
	$upd_qs_active_opta = "UPDATE question_active 
						SET 
	...
	$upd_qs_active_optb = "UPDATE question_active 
						SET 
	...
	$upd_qs_active_optc = "UPDATE question_active 
						SET 
	...
	$upd_qs_active_optd = "UPDATE question_active 
						SET 
	...
	$upd_qs_active_opte = "UPDATE question_active 
						SET 
	...
	$upd_qs_active_optf = "UPDATE question_active
						SET
	...
	$upd_qs_active_optg = "UPDATE question_active
						SET
	...
	$upd_qs_active_opth = "UPDATE question_active
						SET
