<?php
    ...
    if (isset($_REQUEST["btnUploadSoal"])){
        $rnum = rand(111, 99999999);
        $idrand = strtoupper(substr(md5($_SESSION["uSeRiD"]."SQM".$rnum."main.php"),0,10)) ;
        $_SESSION["prevpage"] = "main.php";
        $_SESSION["rnum"] = $rnum;

        echo "<script language=\"javascript\">window.open('home_upload_soal.php?rnum=$rnum&idrand=$idrand','uploadsoal','height=600,width=1000,top=30,left=150,center=yes,toolbar=no,scrollbars=yes,status=no,menubar=no,location=no');</script>";
