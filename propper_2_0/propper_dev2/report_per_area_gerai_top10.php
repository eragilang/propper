        ...
        if (($area != "0") && ($area != "")){
        	...
        	$query = "SELECT 
					...
					WHERE 
						period = $period AND
						set_id = 1 and
						unit_id in (SELECT unit_id FROM pnp.t_unit_branch WHERE unit_type = 'gerai' and area = $area) AND
						employee_id like '0%'
					ORDER BY 
						score DESC, 
						duration
		...
		<div id="page_content_assess"><!--content-->
			<div id="left_column"><!--left column-->
 			...
 			if (($area != "0") && ($area != "")){
				...
				echo "<table style='margin-right:5px;'>
						<caption>Top 20 Area $area</caption>
                	<thead>	
				      <tr>
					  	<th width='45%'>Nama</th>
					  	<th width='15%'>NIK</th>
	            ...
	            $logger->setDBConnection($gatepnp);
	     		...
	     		$logger->Logging(HistoryType::HISTORYTYPE_VIEW_REPORT_GERAI_AREA_TOP10,$userid,"View Report Top 10 per Area Gerai $area Periode $selmonth - $selyear",$ipaddress);
					}
					?>   	
			</div><!--left column-->
			...
			<div id="right_column"><!--right column-->
  				<div class="section"><!--latest news-->
  					...
  					<form action="report_per_area_gerai_top10.php" method="post">
  					<?php
				try{
