        ...
        if ($unitid != "0"){
        	...
        	if ($period < date("Ym")){
        		$cssql ="SELECT 
						...
						WHERE 
							unit_id = ".$unitid." and 
							unit_type IN ('regional') and 
							unit_name IN ('REG SM JABODETABEK WEST','REG SM JATIM','REG SOS BALI NUSRA','REG SOS CENTRAL JABOTABEK','REG SOS JABAR','REG SOS JATENG','REG SOS SULAWESI','REG SOS SUMBAGSEL','REG SOS SUMBAGTENG','REG SOS SUMBAGUT') and
							period = '$period' and 
			...
			<div id="right_column"><!--right column-->
  				<div class="section"><!--latest news-->
					...
					?>
  					...
  					<form action="report_per_bes.php" method="post">
  					<?php
				try{
  					$channel = "SELECT 
					  			unit_id, 
					  		...
					  		FROM 
							  	csdm2.t_unit_branch 
						  	WHERE 
							  	unit_type IN ('regional')
								AND unit_flag = 1
								AND unit_name IN ('REG SM JABODETABEK WEST','REG SM JATIM','REG SOS BALI NUSRA','REG SOS CENTRAL JABOTABEK','REG SOS JABAR','REG SOS JATENG','REG SOS SULAWESI','REG SOS SUMBAGSEL','REG SOS SUMBAGTENG','REG SOS SUMBAGUT')
						    ORDER BY 2 asc";
