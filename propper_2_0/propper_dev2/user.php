				   ...
				   <label for="username"></label>
 					...
 					<table border="0" align="center" cellpadding="7" cellspacing="0"> 
 								...
 								<table border="0" align="center" cellpadding="7" cellspacing="0"> 
 									<tr>
 										...
 										<td>
 											...
 											<?php 
 												$conn = mysql_connect("10.250.191.137","pnp","pnp123") or die ("Error");                                                                                                                                                                                    
												mysql_select_db("pnp");
												
												// $conn = mysql_connect("10.250.193.238","apps","B4nC1@K35uRuP4nkwoK") or die ("Error");                                                                                                                                                                                    
												// mysql_select_db("pnp");
												
 												$slc_assess_type = "SELECT assess_id, assess_desc FROM assess_type WHERE ENABLE = '1' and assess_id NOT IN ('H4J', 'HAJ') ORDER BY assess_id DESC;"; 												
 											//	$slc_assess_type = "SELECT assess_id, assess_desc FROM assess_type WHERE ENABLE = '1' ORDER BY assess_id DESC;";
									...
									<tr>	 								
 										<td>Username </td> 
 							 	 		<td> <input type="text" name="nama" size="35" maxlength="26"/> </td> 
 							 	 	</tr>								 									
 									<tr>	 								
 										<td>MSISDN (62)</td> 
 							 	 		<td> <input type="text" name="msisdn" size="35" maxlength="26"/> </td> 
 							 	 	</tr>
 								</table>	
 						...
 						<tr>
 								...
 								<?php
									...
									if($_POST['Submit']){
									  ...
									  $msisdn = TRIM($_POST['msisdn']);		
									 	...
									 	$slc_pass = "SELECT LPAD(CEIL(RAND()*(999999+1)),6,'0');"; 
									 	$res_slc_pass = mysql_query($slc_pass);
									 	$row_slc_pass = mysql_fetch_array($res_slc_pass);
									 	$password = $row_slc_pass[0];
										...
										$jml_unitType = mysql_num_rows($res_slc_unitType);
									 	...
									 	if ($jml_unitType == 1) { 										  
									 		...
									 		if ($clean_nama == "") {
									 			...
									 			$create="INSERT INTO pnp.assess_user(user_id,password,user_name,unit_type,assess_id,acc_status,msisdn) 
									 						VALUES('$username','$password','$nama','$unitType','$userpnp','1','$msisdn')";
									 			#echo "$create";
   								 			$create_user = mysql_query($create);                                                                                                                                                                                                                
		  						 			if (! $create_user)                                                                                                                                                                                                                             
									 			die("Cannot query: $create_user");   
									 		
									 			echo "<b class=style2>User yang telah di daftarkan : <br>User Name : <i>$username</i> <br>Password   : <i>$password</i></b> <br><br>";
						    ...
						    <li class="style1">Username Max 26 karakter.</li>
