...
<?php
	...
	if ($set_id == 3) {
		//$job_function = 5;
		$slc_lvl_3 = "SELECT DISTINCT LEVEL FROM cc_assess_type WHERE assess_id = '".$assess_id."';"; 
	...
	if ($set_id == 1) {
		...
		$upd_ass_period = "UPDATE cc_assess_period SET start_assess = ".$start_assess.", end_assess = ".$end_assess." WHERE unit_id IN ".$unit_id." AND LEVEL = '".$job_function."';";
	} else if ($set_id == 2) {
		$upd_ass_period = "UPDATE cc_assess_period SET start_assess = ".$start_assess.", end_assess = ".$end_assess." WHERE unit_type = 'callcenter' AND LEVEL = '".$job_function."';";
	} else if ($set_id == 3) {
		$upd_ass_period = "UPDATE cc_assess_period SET start_assess = ".$start_assess.", end_assess = ".$end_assess." WHERE unit_id = '0' AND LEVEL = '".$job_function."';";	
	}
	...
	if (($baseName != "") && ($extension == "csv")) {
		...
		$chk_qs_active 	= "SELECT * FROM cc_question_active"
		...
		if ($row < 1) {
			...
			if (($handle = fopen($file, "r")) !== FALSE) {
				echo "BUKA FILE";
				$load_qs_temp = "INSERT INTO cc_question_temp_".$fileUpload_name." VALUES ";
				...
				$create_tb_qs_temp = "CREATE TABLE `cc_question_temp_".$fileUpload_name."` (
										`questions` text,
										`sola` text,
										`solb` text,
										`solc` text,
				...
				$res_create_tb_qs_temp = mysql_query($create_tb_qs_temp);
				...
				if ($res_create_tb_qs_temp) {
					...
					if ($res_load_qs_temp) {
						echo "INSERT QUESTION TEMP";
						// check jml soal
						
						//check min Soal
						$chk_min_soal = "SELECT DISTINCT max_question FROM cc_assess_param WHERE level = '".$job_function."' AND set_id = '".$set_id."';";
						$res_chk_min_soal = mysql_query($chk_min_soal);
						$row_chk_min_soal = mysql_fetch_array($res_chk_min_soal);
						$min_soal = $row_chk_min_soal[0];
						
						$chk_jml_soal = "SELECT * FROM cc_question_temp_".$fileUpload_name.";";
						...
						if ($jml_soal > $min_soal) {
							...
							$ins_qs_active 	= "INSERT INTO cc_question_active 
											   ...
											   ." mandatory,
											   ".$regional." 
											   FROM cc_question_temp_".$fileUpload_name.";";
											   
							// Insert to question_active														
							$res_ins_qs_active = mysql_query($ins_qs_active);
							
							if ($res_ins_qs_active) {
								echo "MASUK QUESTION ACTIVE";
								// delete tabel question_temp_...
								$dlt_tbl_question_temp = "DROP TABLE cc_question_temp_".$fileUpload_name;
								
								// Delete tabel question_temp_...
								$res_dlt_tbl_question_temp = mysql_query($dlt_tbl_question_temp);
								
								// Bersihkan jawaban dari .
								$upd_qs_active = "UPDATE cc_question_active SET 
								...
								$upd_qs_active_qst = "UPDATE cc_question_active 
													SET 
								...
								$upd_qs_active_opta = "UPDATE cc_question_active 
													SET 
								...
								$upd_qs_active_optb = "UPDATE cc_question_active 
													SET 
								...
								$upd_qs_active_optc = "UPDATE cc_question_active 
													SET 
								...
								$upd_qs_active_optd = "UPDATE cc_question_active 
													SET 
								...
								$upd_qs_active_opte = "UPDATE cc_question_active 
													SET 
								...
								$upd_qs_active_optf = "UPDATE cc_question_active
													SET
								...
								$upd_qs_active_optg = "UPDATE cc_question_active
													SET
								...
								$upd_qs_active_opth = "UPDATE cc_question_active
													SET
								...
								$upd_cat_dis_0 = "UPDATE cc_category_distribution SET nb_question = '0' 
													WHERE set_id = '".$set_id."' AND
													level = '".$job_function."' AND
													regional = '".$regional."'";
								// Set nb_question = 0 pada category_distribution
								$res_upd_cat_dis_0 = mysql_query($upd_cat_dis_0);
								#echo "\n".$upd_cat_dis_0."\n";
								if($res_upd_cat_dis_0) {
									echo "CHECK KATEGORI SOAL";
									// check question distribution
									$slc_qs_active = "SELECT cat_group, count(*), sum(is_mandatory),"
														." group_description FROM cc_question_active qa, cc_category ca"
									...
									if ($jml_cat_soal > 0) {
										// max soal
										$slc_max_soal = "SELECT max_question FROM cc_assess_param WHERE set_id = '".$set_id."' AND LEVEL = '".$job_function."';";
										...
										for ($i = 0; $i < count($cat_id); $i++) {
											...
											$upd_cat_dis = "UPDATE cc_category_distribution SET"
