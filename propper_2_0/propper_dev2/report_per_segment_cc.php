			...
			<div id="left_column"><!--left column-->
				<div id="sectiond">
  					<div id="userinfos">
						...
						<a href="report_qao.php">QAO &amp; Webservice</a>		  			</div>
					  <div id="userscores">
...
//echo $cssql;
 			if ($unitid != "0"){
			   ...
			   <tr align="right">
	            ...
	            $logger->setDBConnection($gatepnp);
	     		...
	     		$logger->Logging(HistoryType::HISTORYTYPE_VIEW_REPORT_GRAPARI,$userid,"View Report Per Segment $selsegment Call Center $unitid Periode $selmonth - $selyear",$ipaddress);
			...
			<div id="right_column"><!--right column-->
  				<div class="section"><!--latest news-->
  					...
  					<form action="report_per_segment_cc.php" method="post">
  					<?php
				try{
  					$channel = "SELECT 
					  			unit_id, 
					  		...
					  		FROM 
							  	csdm2.t_unit_branch 
						  	WHERE 
							  	unit_type IN ('callcenter' and 'webservice')
								AND unit_flag = 1
						    ORDER BY 2 asc";
					...
					echo "</select>\n";
  					...
  					$job = "SELECT 
								...
								job_fullname 
					  		FROM 
							  	csdm2.t_employee_jobfunction 
						  	WHERE 
							  	pnp_assessee = '1' and
							  	job_type_callcenter = 'y'
						    ORDER BY 2 asc";
						    
				    $jobres = $gatecsdm->ExecuteQuery($job,"csdm");
				    
				    echo "<select name='segmen' style='margin-left:10px;padding:2px;font-family:\"Trebuchet MS\", Arial;font-size:13px;width:180px;'>\n";
            		
					while($row = mysql_fetch_row($jobres)){
                		if ($selsegment == $row[0]){
                    		echo "<option value='".$row[0]."' style='margin:3px;' selected>".$row[1]."</option>\n";
                		}
                		else{
