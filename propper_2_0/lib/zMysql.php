...
function getMenuId($dbname,$action) {
	$qMenu = selectQuery($dbname,'menu',"id",
		"action='".$action."'");
	$resMenu = fetchData($qMenu);
	return $resMenu[0]['id'];
}
 ...
 * Get Menu Path
 */
function getMenuRoad($dbname,$menuId,$last=1) {
	$str = '';
	$qMenu = selectQuery($dbname,'menu',"parent,caption",
		"id=".$menuId);
	$resMenu = fetchData($qMenu);
	
	$str .= $resMenu[0]['caption'];
	if($last==0) {
		$str .= ' > ';
	}
	if($resMenu[0]['parent']!='0') {
		$str = getMenuRoad($dbname,$resMenu[0]['parent'],0).$str;
	}
