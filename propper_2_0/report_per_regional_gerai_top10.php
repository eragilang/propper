        ...
        if (($regional != "0") && ($regional != "")){
        	...
        	$query = "SELECT 
					...
					WHERE 
						period = $period AND
						set_id = 1 and
						unit_id in (SELECT unit_id FROM pnp.t_unit_branch WHERE unit_type = 'gerai' and regional = $regional) AND
						employee_id like '0%'
					ORDER BY 
						score DESC, 
						duration
		...
		<div id="page_content_assess"><!--content-->
			<div id="left_column"><!--left column-->
 			...
 			if (($regional != "0") && ($regional != "")){
	            ...
	            $logger->setDBConnection($gatepnp);
	     		...
	     		$logger->Logging(HistoryType::HISTORYTYPE_VIEW_REPORT_GERAI_REGIONAL_TOP10,$userid,"View Report Top 10 per Regional Gerai $regional Periode $selmonth - $selyear",$ipaddress);
					}
					?>   	
			</div><!--left column-->
			...
			<div id="right_column"><!--right column-->
  				<div class="section"><!--latest news-->
  					...
  					<form action="report_per_regional_gerai_top10.php" method="post">
  					<?php
				try{
  					$channel = "SELECT 
					  				reg_code, 
