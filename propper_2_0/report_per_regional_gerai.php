		...
		<div id="page_content_assess"><!--content-->
			<div id="left_column"><!--left column-->
 			...
 			if (($regional != "0") && ($regional != "")){
				...
				echo "<table style='margin-right:5px;'>
						<caption>Regional $regional</caption>
                	<thead>	
				      <tr>
    			...
    			foreach ($arrOfGeraiKPI as $unitid => $kpiinfo){
    				list($gname, $nbcsr, $kpicsrsms1,$kpicsrsms2, $kpicsr, $kpi) = split("#", $kpiinfo);
    				echo "<tr>
    							<th>$gname</th>
    							<td>$nbcsr</td>
    							<td>$kpicsr</td>
	            ...
	            $logger->setDBConnection($gatepnp);
	     		...
	     		$logger->Logging(HistoryType::HISTORYTYPE_VIEW_REPORT_GERAI_REGIONAL,$userid,"View Report Per Regional Gerai $regional Periode $selmonth - $selyear",$ipaddress);
					}
					?>   	
			</div><!--left column-->
			...
			<div id="right_column"><!--right column-->
  				<div class="section"><!--latest news-->
  					...
  					<form action="report_per_regional_gerai.php" method="post">
  					<?php
				try{
  					$channel = "SELECT 
					  				reg_code, 
