...
<script>
	...
	function validatingJumlah(jumlah) {
		if(jumlah.value != 100) {
			jumlah.style.color = '#ce2e0b';
		} else {
			jumlah.style.color = '#2c2523';
		}
	}
	
	function validatingEntry(entry) {
		if(entry.value % 10 != 0) {
			entry.style.color = '#ce2e0b';
		} else {
			entry.style.color = '#2c2523';
		}		
	}
	
	function is100(jumlah) {
		if(jumlah.value == 100) {
			return true;
		} else {
			return false;
  ...
  <tr>
	...
	<table width="600" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
      ...
      <tr>
		...
		<form id="formSubmit" name="form" action="process_upload_soal.php" method="post" enctype="multipart/form-data">
...
function getChange(){
			...
			if (type=="GRAPARI"){
				...
				$('#formSubmit').attr('action', '/propper/proses_upload_propper.php');
				document.getElementById("bobot_cc").style.display = "none";
				document.getElementById("head_cc").style.display = "none";
				document.getElementById("proses_cc").style.display = "none";
				document.getElementById("produk_cc").style.display = "none";
...
function calJumlah() {
	...
	{
		validatingJumlah(jml_fos_gpr)
	}
	
	if (isSuffixZero(tl_gpr_proses) && isSuffixZero(tl_gpr_produk) && isSuffixZero(tl_gpr_perilaku) &&
	isSuffixZero(spv_gpr_proses) && isSuffixZero(spv_gpr_produk) && isSuffixZero(spv_gpr_perilaku) &&
	isSuffixZero(cs_gpr_proses) && isSuffixZero(cs_gpr_produk) && isSuffixZero(cs_gpr_perilaku) && 
	is100(jml_tl_gpr) && is100(jml_spv_gpr) && is100(jml_cs_gpr) && is100(jml_fos_gpr)) {
...
function calJumlah1() {
	...
	{
		validatingJumlah(jml_cs_cc)
	}
	if (isSuffixZero(tl_cc_proses) && isSuffixZero(tl_cc_produk) && isSuffixZero(tl_cc_perilaku) &&
	isSuffixZero(spv_cc_proses) && isSuffixZero(spv_cc_produk) && isSuffixZero(spv_cc_perilaku) &&
	isSuffixZero(cs_cc_proses) && isSuffixZero(cs_cc_produk) && isSuffixZero(cs_cc_perilaku) &&
	is100(jml_tl_cc) && is100(jml_spv_cc) && is100(jml_cs_cc)) {
		$(':input[type="submit"]').prop('disabled', false);
	} else {
		$(':input[type="submit"]').prop('disabled', true);
	}
...
function calJumlah2() {
	...
	{
		validatingJumlah(jml_cho_ec)
	}
	if (isSuffixZero(tl_ec_proses) && isSuffixZero(tl_ec_produk) && isSuffixZero(tl_ec_perilaku) && isSuffixZero(cho_ec_proses) &&
	isSuffixZero(spv_ec_proses) && isSuffixZero(spv_ec_produk) && isSuffixZero(spv_ec_perilaku) && isSuffixZero(cho_ec_produk) &&
	isSuffixZero(cs_ec_proses) && isSuffixZero(cs_ec_produk) && isSuffixZero(cs_ec_perilaku) && isSuffixZero(cho_ec_perilaku) &&
	is100(jml_tl_ec) && is100(jml_spv_ec) && is100(jml_cs_ec) && is100(jml_cho_ec)) {
		$(':input[type="submit"]').prop('disabled', false);
	} else {
		$(':input[type="submit"]').prop('disabled', true);
	}
